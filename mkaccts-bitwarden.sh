...
echo "Creating user: wallsmason77"
sudo useradd -m -u 1005 -s /bin/bash --comment "Mason Walls" wallsmason77
PASSWORD=$(get_password "wallsmason77")
if [ -n "$PASSWORD" ]; then
  echo "wallsmason77:$PASSWORD" | sudo chpasswd
  sudo usermod -aG sudo wallsmason77
else
  echo "Warning: Could not retrieve password for wallsmason77"
fi
